...
def check_git_status():
    ...
    try:
        ...
        if n > 0:
            s = f"鈿狅笍 WARNING: code is out of date by {n} commit{'s' * (n > 1)}. " \
                f"Use 'git pull' to update or 'git clone {url}' to download latest."
        else:
            s = f'up to date with {url} 鉁�'
...
def check_requirements(requirements='requirements.txt', exclude=()):
    ...
    if n:  # if packages updated
        ...
        s = f"{prefix} {n} package{'s' * (n > 1)} updated per {source}\n" \
            f"{prefix} 鈿狅笍 {colorstr('bold', 'Restart runtime or rerun command for updates to take effect')}\n"
        print(emojis(s))  # emoji-safe
...
def clean_str(s):
    ...
    return re.sub(pattern="[|@#!隆路$鈧�%&()=?驴^*;:,篓麓><+]", repl="_", string=s)
